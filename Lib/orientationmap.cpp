...
void OrientationMap::setParams(const std::string path, int blockSize, int gaussBlockBasic, float gaussSigmaBasic, int gaussBlockAdv, float gaussSigmaAdv){

    GAUSSIAN_BLUR_SETTINGS gaussianBlurBasic{gaussBlockBasic, gaussSigmaBasic};
    GAUSSIAN_BLUR_SETTINGS gaussianBlurAdvanced{gaussBlockAdv, gaussSigmaAdv};
    OMAP_PARAMS omap_params{blockSize, gaussianBlurBasic, gaussianBlurAdvanced};
    setParams(cv::imread(path,cv::IMREAD_GRAYSCALE),omap_params);
}

void OrientationMap::setParams(const std::string path, OMAP_PARAMS omap){
    setParams(cv::imread(path),omap);
}

void OrientationMap::setParams(const cv::Mat &imgFingerprint, OMAP_PARAMS omapParams)
{
    if(imgFingerprint.cols != 256 || imgFingerprint.rows != 364){
        cv::resize(imgFingerprint, this->imgInput,cv::Size(256,364));
        cv::imshow("ddd",this->imgInput);
    } else {
...
void OrientationMap::drawBasicMap(std::string path){
    drawBasicMap(cv::imread(path,cv::IMREAD_GRAYSCALE));
}
